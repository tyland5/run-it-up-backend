...
router.post('/makePost', [verifyCSRF, upload.array('media', 12)], async (req, res)=>{
    ...
    try{
        ...
        for(let i =0; i < req.files.length; i ++){
            ...
            await pool.execute(sql2, values2)
        }
    }
    catch (e){
        console.log(e)
...
router.get('/getPosts', checkIfLoggedIn, async(req, res)=>{
...
router.get('/getComments', checkIfLoggedIn, async(req,res) =>{
...
router.post('/postComment', verifyCSRF, async(req,res) =>{
    ...
    try{
        ...
        const [result] = await pool.execute(sql, [postId, req.session.uid, comment])
...
router.post('/deletePost', verifyCSRF, async(req, res)=>{
    ...
    try{
        ...
        const [result2] = await pool.execute(sql2, [post_id])
